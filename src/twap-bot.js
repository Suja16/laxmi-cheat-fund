...
// TWAP order enum-like
const TWAPOrderStatus = {
  PENDING: 'PENDING',
  ACTIVE: 'ACTIVE',
  FILLED: 'FILLED',
...
export class UltraRealisticTWAPStrategy {
  ...
  setConfiguration(config) {
    this.config = {
      ...
      maxExecutionTime: config.maxExecutionTime || 24, // 24 hours max
      
      // Ultra-realistic TWAP parameters
    ...
    console.log(`  Pair: ${this.config.fromTokenSymbol}/${this.config.toTokenSymbol}`);
  ...
  generateExecutionSchedule() {
    ...
    for (let i = 0; i < this.config.numberOfOrders; i++) {
      ...
      if (this.config.adaptiveTiming) {
        ...
        const volatilityAdjustment = this.currentVolatility > this.config.volatilityThreshold ? 1.5 : 1.0;
        const trendAdjustment = Math.abs(this.trendHistory.slice(-5).reduce((a, b) => a + b, 0) / 5) > this.config.trendThreshold ? 1.3 : 1.0;
        
        intervalMinutes = Math.floor(intervalMinutes * volatilityAdjustment * trendAdjustment);
        intervalMinutes = Math.max(this.config.minOrderInterval, Math.min(intervalMinutes, this.config.maxOrderInterval));
      }

      const executionTime = new Date(startTime.getTime() + (i * intervalMinutes * 60 * 1000));
      this.executionSchedule.set(i, executionTime);
      
      console.log(`  Order ${i + 1}: ${executionTime.toLocaleString()} (${intervalMinutes}min interval)`);
    }
  }

  displayTWAPSummary() {
    const orderAmount = parseFloat(this.config.totalAmount) / this.config.numberOfOrders;
    const totalDuration = (this.config.numberOfOrders * this.config.intervalMinutes) / 60;

    console.log('\n📋 Ultra-Realistic TWAP Strategy Summary:');
    console.log('==========================================');
    console.log(`  📊 Total Orders: ${this.config.numberOfOrders}`);
    console.log(`  💰 Amount per Order: ${orderAmount.toFixed(6)} ${this.config.fromTokenSymbol}`);
    console.log(`  ⏱️  Average Interval: ${this.config.intervalMinutes} minutes`);
  ...
  updateAdaptiveOrderSize(orderIndex) {
    ...
    const baseOrderSize = parseFloat(this.config.totalAmount) / this.config.numberOfOrders;
  ...
  async getUserConfiguration() {
    ...
    try {
      console.log('📝 Ultra-Realistic TWAP Strategy Configuration');
      console.log('==============================================\n');
      ...
      this.config.numberOfOrders = parseInt(await question('Number of Orders (e.g., 8): '));
      ...
      const slippageInput = await question('Slippage Tolerance % (e.g., 0.5 for 0.5%): ');
      this.config.slippageTolerance = parseFloat(slippageInput || '0.5');
  ...
  async createTWAPOrders() {
    ...
    const orders = [];
    const baseOrderAmount = parseFloat(this.config.totalAmount) / this.config.numberOfOrders;
    ...
    for (let i = 0; i < this.config.numberOfOrders; i++) {
      try {
        ...
        if (orderData) {
          ...
          console.log(`   💰 Order Size: ${adaptiveOrderSize.toFixed(6)} ${this.config.fromTokenSymbol}`);
        ...
        console.error(`❌ Failed to create TWAP order ${i + 1}:`, error.message);
  ...
  async createSingleTWAPOrder(makingAmount, orderIndex, executionTime, expirationTime) {
    try {
      ...
      const orderSize = parseFloat(ethers.utils.formatUnits(makingAmount, this.config.fromTokenDecimals));
      ...
      // Sign the order using ethers v5 compatible method
    ...
    const monitoringInterval = setInterval(async () => {
      try {
        ...
        const activeOrdersCount = Array.from(this.activeOrders.values())
          .filter(order => order.status === TWAPOrderStatus.ACTIVE).length;
      ...
      try {
        ...
        if (status) {
          ...
          if (remainingAmount.isZero()) {
            ...
            console.log(`✅ TWAP Order ${orderData.orderIndex + 1} filled completely!`);
            ...
            console.log(`🔄 TWAP Order ${orderData.orderIndex + 1} partially filled`);
          }
        }
      } catch (error) {
        console.log(`⚠️ Could not check status for TWAP order ${orderData.orderIndex + 1}`);
      }
    }
  }
  ...
  async handleOrderExpirations() {
    ...
    for (const [, orderData] of this.activeOrders) {
      if (orderData.status === TWAPOrderStatus.ACTIVE && orderData.expiresAt <= now) {
    ...
    for (const [, orderData] of this.activeOrders) {
      try {
        ...
        if (orderData.limitOrderInstance) {
          ...
          successCount++;
          console.log(`✅ TWAP Order ${orderData.orderIndex + 1} submitted to 1inch`);
          console.log(`   📊 Order Hash: ${orderData.orderHash.slice(0, 10)}...`);
        } else {
          console.log(`❌ Failed to submit TWAP order ${orderData.orderIndex + 1}: No limit order instance available`);
        }
        
      } catch (error) {
        console.error(`❌ Submit error for TWAP order ${orderData.orderIndex + 1}:`, error.message);
        this.executionFailures++;
  ...
  displayTradingStats() {
    ...
    const successRate = this.totalTrades > 0 ? ((this.totalTrades - this.executionFailures) / this.totalTrades) * 100 : 0;
    ...
    console.log(`Slippage Events: ${this.slippageEvents}`);
    ...
    console.log(`Average Order Size: ${stats.averageOrderSize} ${this.config.fromTokenSymbol}`);
    if (stats.nextExecutionTime) {
      console.log(`Next Execution: ${stats.nextExecutionTime.toLocaleString()}`);
    }
  }
  ...
  getStrategyStats() {
    ...
    const stats = {
      ...
      activeOrders: orders.filter(o => o.status === TWAPOrderStatus.ACTIVE).length,
      filledOrders: orders.filter(o => o.status === TWAPOrderStatus.FILLED).length,
      expiredOrders: orders.filter(o => o.status === TWAPOrderStatus.EXPIRED).length,
      cancelledOrders: orders.filter(o => o.status === TWAPOrderStatus.CANCELLED).length,
