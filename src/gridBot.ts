...
export class VolatilityGridStrategy {
    ...
    public gridLevels: Map<number, { buyPrice: number; sellPrice: number }> = new Map();
    ...
    public setConfiguration(config: StrategyConfig): void {
        ...
        this.config = {
            ...
            quoteAmount: config.totalQuoteAmount, // For grid, we need both base and quote amounts
            gridLevels: config.gridLevels || 10,
            priceRange: config.priceRangePercent || 20, // Use price range as range %
            currentPrice: 0, // Will be fetched
            slippageTolerance: 1, // Default slippage
            gasPrice: 'auto',
    ...
    private async getUserConfiguration(): Promise<void> {
        ...
        try {
            ...
            // Token Configuration
            const baseToken = await question('Base Token Address (or press enter for WETH): ') || 
                process.env.DEFAULT_FROM_TOKEN || '0x7ceb23fd6bc0add59e62ac25578270cff1b9f619';
            const quoteToken = await question('Quote Token Address (or press enter for USDC): ') ||
                process.env.DEFAULT_TO_TOKEN || '0x3c499c542cEF5E3811e1192ce70d8cC03d59';
    ...
    private async getTokenInfo(tokenAddress: string): Promise<{ symbol: string; decimals: number }> {
        try {
            const tokenContract = ethersCompat.getContract(tokenAddress, ERC20_ABI, this.provider);
